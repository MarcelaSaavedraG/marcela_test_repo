...
```{r}
garden_harvest %>% 
  filter(vegetable %in% "beets") %>% 
  group_by(variety, date) %>% 
  summarise(wt_var_day= sum(weight)) %>% 
  mutate(wt_lbs=  wt_var_day* 0.00220462,
         cum_wt= cumsum(wt_lbs)) %>% 
...
garden_harvest %>% 
  ...
  ggplot(aes(y=vegetable,x=wt_lbs))+
  geom_boxplot()
...
babynames %>% 
  ...
  filter(has2000 == TRUE) %>% 
  group_by(year, sex) %>% 
  summarise(name=name, proportion = n/sum(n))
```
...
babynames %>% 
  ...
  filter(has2000 == TRUE) %>% 
  group_by(sex) %>% 
  mutate(totalnames2000_overyears= sum(n)) %>% 
  group_by(year) %>% 
  mutate(prop_names_per_year = sum(n)/totalnames2000_overyears) %>% 
  ggplot()+
  geom_line(aes(x=year,y=prop_names_per_year,line=sex,col=sex))+
  labs(x="",y="Proportion",title="Proportion of names with more than 2000 uses, as measured through the years")
...
```{r}
#I had initially done this but found out later theres another way to find disctint
# q10 <- babynames %>% 
#   group_by(year, sex) %>% 
#   mutate(proport = n/sum(n)) %>% 
#   filter(proport < 0.1) %>% 
#   summarise(unusual_names_per_year= n())
# 
# q10 %>% 
#   ggplot(aes(x=year,y=unusual_names_per_year))+
#   geom_col(col="lightblue")+
#   facet_wrap(~sex)+
#   labs(title="Number of unusual names per year (proportion < 0.1), per sex",x="",y="Amount of unusual names")+
#   theme_clean()
...
covid19 %>% 
  ...
  group_by(state) %>% 
  ggplot(aes(y=cases,x=date, colour=state)) +
  geom_line()+
  scale_y_continuous(labels = scales::comma)+
  labs(x=" ",y="Cumulative cases")
```
This is not a fair comparison given that North and South Dakota have a much smaller population.
...
covid19 %>% 
  ...
  labs(x=" ",y="Cumulative cases")
```
It seems more fair now given that we have accounted for changes in the magnitudes of the cumulative cases. It would be fair to do a per capita count.
...
newcovid19 <- covid19 %>% 
  group_by(state) %>% 
  mutate(one_lag = lag(cases),
            seven_lag= lag(cases, n=7)) %>% 
  replace_na(list(one_lag=0,seven_lag=0)) %>% 
  mutate(new_cases_per_day= cases - one_lag,
         average_7_day= (cases-seven_lag)/7)
```
...
newcovid19 %>% 
  ggplot(aes(x=date)) + 
  geom_line(aes(y=new_cases_per_day)) +
  geom_line(aes(y=average_7_day),col="blue") +
  facet_wrap(~state)
```
  I notice that the scales are  the same for all the states, thus making states with low cases seem like almost a straight line, which is not very communicative of the covid situation per state.
...
```
We are able to compare based on geographic context. Furthermore, we are able to change the y scale of each graph, which enables us to better compare the situation by state. In the previous plot since all the scales were the same, states with fewer cases would get overriden by those with more scales. 
